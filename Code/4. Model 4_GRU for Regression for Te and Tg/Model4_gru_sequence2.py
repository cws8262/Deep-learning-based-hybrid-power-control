...
with tf.variable_scope("GRU") as gru_v_scope:
  Mcell= tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.GRUCell(num_units=60, activation=tf.tanh) for _ in range(1)])
...
with tf.Session() as sess:
   ...
   variables_names = [v.name for v in tf.trainable_variables() if v.name.startswith(gru_v_scope.name)]
   values = sess.run(variables_names)
   i=0
   gru_wb = np.zeros((4,), dtype=np.object)
   for k, v in zip(variables_names, values):
       gru_wb[i]=v;
       i=i+1

   sio.savemat('gru_wb_2.mat', {'gru_wb':gru_wb})
   ...
   sio.savemat('fcl_wb_2.mat', {'fcl_wb' :fcl_wb})
   sio.savemat('xmean_2.mat',{'xmean' :Xmean_})
   sio.savemat('xsig_2.mat',{'xsig' :Xsig_})
   sio.savemat('ymean_2.mat',{'ymean' :Ymean})
   sio.savemat('ysig_2.mat',{'ysig' :Ysig})