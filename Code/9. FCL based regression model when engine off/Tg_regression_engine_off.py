...
input_=sio.loadmat('engoff_train_X.mat',squeeze_me=False)
input_=np.array(input_['X_for_engoff'])

output_=sio.loadmat('engoff_train_Y.mat',squeeze_me=False)
output_=np.array(output_['y_for_engoff'])

X_train=input_
y_train=output_


input_=sio.loadmat('engoff_test_X.mat',squeeze_me=False)
input_=np.array(input_['X_for_engoff'])

output_=sio.loadmat('engoff_test_y.mat',squeeze_me=False)
output_=np.array(output_['y_for_engoff'])
...
regularizers = tf.nn.l2_loss(W1) + tf.nn.l2_loss(W2) + tf.nn.l2_loss(W3) + tf.nn.l2_loss(W4) + tf.nn.l2_loss(W5) + tf.nn.l2_loss(W6) + tf.nn.l2_loss(W7) + tf.nn.l2_loss(W8)
...
with tf.Session() as sess:
   sess.run(init)
   # saver.restore(sess, "Weighting_factor_engineoff.ckpt")
   batch=512
   ...
   for step in range(300):
      ...
      if (step+1) % 10 == 0:
         save_path = saver.save(sess, "Weighting_factor_engineoff.ckpt")
   ...
   for k, v in zip(variables_names, values):
       ...
       fcl_wb[i]=v;
       i=i+1
   sio.savemat('fcl_wb_engoff.mat', {'fcl_wb' :fcl_wb})

   sio.savemat('xmean_engoff.mat',{'xmean' :Xmean})
   sio.savemat('xsig_engoff.mat',{'xsig' :Xsig})
   sio.savemat('ymean_engoff.mat',{'ymean' :Ymean})
   sio.savemat('ysig_engoff.mat',{'ysig' :Ysig})