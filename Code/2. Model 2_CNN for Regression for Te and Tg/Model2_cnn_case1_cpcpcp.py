...
with tf.variable_scope("CPL") as cpl_v_scope:
	...
	L1_cnn = tf.nn.max_pool(L1_cnn, ksize=[1, l1_pool_size, l1_pool_size, 1],strides=[1, 2, 2, 1], padding='SAME')
	...
	L2_cnn = tf.nn.max_pool(L2_cnn, ksize=[1, l2_pool_size, l2_pool_size, 1],strides=[1, 2, 2, 1], padding='SAME')
...
with tf.Session() as sess:
   sess.run(init)
   # saver.restore(sess, "Weighting_factor_case1.ckpt")
   batch = 512
   ...
   costval_tst = sess.run(cost, feed_dict={X:X_test_temp, Y:y_test_temp,drop_out:1.0})
   ...
   for step in range(300):
      ...
      if (step+1) % 10 == 0:
         save_path = saver.save(sess, "Weighting_factor_case1.ckpt")
   ...
   sio.savemat('cpl_wb_case1.mat', {'cpl_wb' :cpl_wb})
   sio.savemat('fcl_wb_case1.mat', {'fcl_wb' :fcl_wb})
   sio.savemat('xmean_case1.mat',{'xmean' :Xmean})
   sio.savemat('xsig_case1.mat',{'xsig' :Xsig})
   sio.savemat('ymean_case1.mat',{'ymean' :Ymean})
   sio.savemat('ysig_case1.mat',{'ysig' :Ysig})
