...
with tf.variable_scope("GRU") as gru_v_scope:
  Mcell= tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.GRUCell(num_units=40, activation=tf.tanh) for _ in range(1)])
...
with tf.Session() as sess:
   ...
   for k, v in zip(variables_names, values):
       gru_wb[i]=v;
       i=i+1
   sio.savemat('gru_wb_3.mat', {'gru_wb':gru_wb})
   ...
   for k, v in zip(variables_names, values):
       fcl_wb[i]=v;
       i=i+1
   sio.savemat('fcl_wb_3.mat', {'fcl_wb_cla' :fcl_wb})

   sio.savemat('xmean_3.mat',{'xmean_cla' :Xmean_})
   sio.savemat('xsig_3.mat',{'xsig_cla' :Xsig_})
